...
import { seasonWeekContestants, seasonWeeks } from "./data/week-3";
import { seedRandomLineups } from "./utils";

export async function seed(knex: Knex) {
  if (Number(process.env.SEED_WEEK) < 3) {
    return;
  }
  ...
  // Update current week.
  await knex<Season>("seasons").update({ currentWeekNumber: 3 }).where({ id: leagues[0].seasonId });
}
